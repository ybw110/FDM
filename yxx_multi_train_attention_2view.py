...
def train_one_epoch(model, train_loader, optimizer, criterion, device, epoch, epochs):
    ...
    for inputs, labels in loop:
        inputs = [input.to(device) for input in inputs] # 双视角
...
def plot_losses_and_accuracies(train_losses, val_losses, train_accuracies, val_accuracies, epochs):
    ...
    ax2.plot(range(epochs), train_accuracies, color=colors['train_acc'], linestyle='-', linewidth=2,
...
def plot_metric(metric_values, metric_name, epochs, color):
    plt.figure(figsize=(12, 6.75))  # 16:9 比例
    ...
    plt.gca().tick_params(width=2, length=6, labelsize=12)  # 增加刻度线宽度和长度，增大标签字体
...
def plot_class_accuracies(all_labels, all_predictions, class_names):
...
def save_metrics_to_csv(results_dir, train_losses, val_losses, train_accs, val_accs, precisions, recalls, f1_scores):
    ...
    with open(file_path, mode='w', newline='') as file:
        ...
        # 写入表头
        writer.writerow(
            ['Epoch', 'Train Loss', 'Val Loss', 'Train Accuracy', 'Val Accuracy', 'Precision', 'Recall', 'F1-Score'])
...
def main():
    ...
    model = MultiViewNet(num_classes=Config.num_classes, num_views=2).to(device)
    ...
    # 保存损失、准确率、精度、召回率、F1-Score为CSV
    save_metrics_to_csv(results_dir, train_losses, val_losses, train_accs, val_accs, precisions, recalls, f1_scores)

    plot_losses_and_accuracies(train_losses, val_losses, train_accs, val_accs, Config.epochs)
    plot_metric(precisions, 'Precision', Config.epochs, metric_colors['precision'])
    plot_metric(recalls, 'Recall', Config.epochs, metric_colors['recall'])
    plot_metric(f1_scores, 'F1 Score', Config.epochs, metric_colors['f1'])
# 生成分类报告
    class_names = ['Severe Under-extrusion', 'Mild Under-extrusion', 'Normal', 'Mild Over-extrusion', 'Severe Over-extrusion']  # 替换为您的实际类别名称
    ...
    with open(os.path.join(results_dir, 'classification_report.txt'), 'w') as f:
        f.write(report)
    # 绘制每个类别的准确率曲线
    plot_class_accuracies(val_labels, val_preds, class_names)
