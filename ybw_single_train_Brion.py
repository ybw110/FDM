...
def train_one_epoch(model, train_loader, optimizer, criterion, device, epoch, epochs, scaler):
    model.train()
    total_loss = 0
    correct = 0
    total = 0
...
def main():
    ...
    model = MultiHeadAttentionNetwork(num_classes=5)
    ...
    for epoch in range(Config.epochs):
        ...
        # 传入 scaler 进行混合精度训练
        train_loss, train_accuracy = train_one_epoch(model, train_loader, optimizer, criterion, device, epoch, Config.epochs, scaler)
        val_loss, val_accuracy, precision, recall, f1, val_preds, val_labels = validate_model(model, val_loader, criterion, device)
