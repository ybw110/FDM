...
import csv
from random_select_par import random_select_para
"""
这个是自动改参数随机的
"""
...
def main():
    DATE = datetime.now().strftime("%Y%m%d%H%M%S") + "dataset_else=print-radom5-5"
    ...
    gcode_list, flowrate, feed_rate = random_select_para(first_run=True)
    print("等待打印机开始打印...")
    while True:
        if check_condition(client):
            print("打印机已开始打印。开始收集数据...")  # 新增的提示语句
            # **在此处立即发送生成的第一次 G-code 参数**
            for command in gcode_list:  # 遍历第一次生成的Gcode_list，发送 G-code 命令到打印机
                response = client.gcode(command)
            ...
            while photo_num < max_photos:
                try:
                    ...
                    if photo_num == max_photos:
                        # 当照片数量达到上限时，改变参数并重新开始循环
                        random.seed(time.time())  # 设置随机种子，确保每次参数不同
                        time.sleep(1)
                        gcode_list, flowrate, feed_rate = random_select_para()
                        for command in gcode_list:  # 遍历新的 Gcode_list,逐条发送 G 代码命令到打印机,并打印命令及响应
                            response = client.gcode(command)
                            print(f"Command '{command}' sent, response:{response}")
                        print(f"Changing parameter to {gcode_list}")
                        time.sleep(7)  # 等待 7 秒,确保新参数生效
                        photo_num = 0
                        break  # 将 photo_num 重置为 0。使用 break 退出内层循环
